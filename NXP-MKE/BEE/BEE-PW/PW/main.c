...
bool Pressure_release=false,last_cycle = false,LOW_LVL_FLAG=false,TRIP_FLAG=false,update_flag=false;
...
fact_sel_mode=2;	
	while(1){
        ...
        case FACTORY_SETTING:
				...
				if(fact_sel_mode==1){
					show_mode_flicker("SPL",FACTORY_SETTING);  //pr not required
					Pressure_release=false;
				}
				if(fact_sel_mode==2){
					show_mode_flicker("MP-SA",FACTORY_SETTING);  // pr required
					Pressure_release=true;
				}
				if(fact_sel_mode==3){
					show_mode_flicker("MPL-PLC",FACTORY_SETTING); //pr required
					Pressure_release=true;
				}
				if(fact_sel_mode==4){
					show_mode_flicker("PR-SA",FACTORY_SETTING);  //pr not required
					Pressure_release=false;
				}
				if(fact_sel_mode==5){
					show_mode_flicker("PR-PLC",FACTORY_SETTING);  //pr not required
					Pressure_release=false;
				}
				if(fact_sel_mode==6){
					show_mode_flicker("SL-SA",FACTORY_SETTING);  //pr required
					Pressure_release=true;
				}
				if(fact_sel_mode==7){
					show_mode_flicker("SL-PLC",FACTORY_SETTING);  //pr required
					Pressure_release=true;
				}
    ...
    if(stroke_completed < stroke){
        ...
        while((last_cycle ? STP_STATUS : ESTP_STATUS) == 0){
           ...
           /* if(KEY1_PRESSED == 0 && KEY2_PRESSED == 0){
                ...
                if(KEY1_PRESSED == 0 && KEY2_PRESSED == 0){
                    ...
                    enter_mode = 1;
                    reset = true;
                    break;  // exit forward motion immediately
                }
            }*/
        ...
        if(!last_cycle){
            ...
            while(HOM_STATUS == 0){
                ...
                // Continuous key check in reverse motion
                if(KEY1_PRESSED == 0 && KEY2_PRESSED == 0){
                    Delay(500);
                    if(KEY1_PRESSED == 0 && KEY2_PRESSED == 0){
											stroke_completed=stroke;
